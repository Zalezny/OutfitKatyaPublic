...
class KatyaFragment : Fragment() {
    ...
    private fun getData(id : String?)  {
        ...
        lifecycle.coroutineScope.launch {
            ...
            var result : List<OutfitWithKatyaTime> = listOf()

            database.withTransaction {
                result = database.outfitDao().getOutfitWithKatyaTime(id!!.toInt())
            }

            val hoursList = result.first().hours

            timeArrayList = arrayListOf()

            for (j in hoursList.size-1 downTo 0)
            {
                val h = hoursList[j].hour
                val m = hoursList[j].minute
                val s = hoursList[j].second

                //date of hours
                val dateString = hoursList[j].date
                ...
                val timeID = hoursList[j].kid

                totalHelper.addTotalTime(h,m,s)
                val element = TimeDataModel(finalDate, time, timeID.toString())
                                            ...
                                            override fun onDismissed(
                                                ...
                                                if(!isUndo)
                                                {
                                                    ...
                                                    deleteFromServer(
                                                        ...
                                                        outfitID,
                                                        IS_KATYA,
                                                        transientBottomBar!!.context.applicationContext
                                                    )
...
//
    ...
    private fun deleteFromServer(timeID : String, outfitID : String, IS_KATYA : Boolean, appContext : Context)
    {
        val name = if(IS_KATYA) KATYA else MOM
        ...
        {
            lifecycle.coroutineScope.launch {
                val katyaTimeDao = (appContext as OutfitApplication).repositoryKatyaTime
                katyaTimeDao.deleteByKatyaTimeId(timeID.toInt())
            }

        }
        //TODO: MOM DELETE TIME
